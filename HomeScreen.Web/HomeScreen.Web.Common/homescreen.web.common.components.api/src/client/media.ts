...
import { zMediaItem, zPaginatedMediaItem } from '../generated/zod.gen';
import { mediaGetResponseTransformer } from '../generated/transformers.gen';
...
export class MediaClient implements IMediaClient {
    ...
    private async *iterateStream<TValue, TValidator extends ZodType<TValue>>(
        ...
        for await (const chunk of stream.values({ preventCancel: false })) {
            signal?.throwIfAborted();
            yield* this.processChunk<TValue, TValidator>(
                validator,
                chunk,
                signal,
            );
