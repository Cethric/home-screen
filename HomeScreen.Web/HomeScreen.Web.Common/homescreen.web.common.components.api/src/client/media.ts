...
export function isAsyncIterable<TValue, TIterable extends Partial<AsyncIterable<TValue>>>(input?: TIterable): boolean {
  if (input === null) {
    return false;
  }
  if (input === undefined) {
...
export interface IMediaClient {
  paginate(offset: number, length: number, signal?: AbortSignal): AsyncGenerator<PaginatedMediaItem>;

  random(count: number, signal?: AbortSignal): AsyncGenerator<MediaItem | undefined>;

  toggle(mediaId: string, enabled: boolean, signal?: AbortSignal): Promise<MediaItem | undefined>;

  downloadLine(direction: number, size: number, signal?: AbortSignal): Promise<FileStreamHttpResult | undefined>;

  transformLine(direction: number, size: number, signal?: AbortSignal): Promise<AcceptedTransformMediaLine | undefined>;
...
export class MediaClient implements IMediaClient {
  ...
  public async *paginate(offset: number, length: number, signal?: AbortSignal): AsyncGenerator<PaginatedMediaItem> {
  ...
  public async *random(count: number, signal?: AbortSignal): AsyncGenerator<MediaItem | undefined> {
    ...
    if (response.response.status === 200) {
      for await (const entry of this.processStream(response.response, zMediaItem, signal)) {
  ...
  public async toggle(mediaId: string, enabled: boolean, signal?: AbortSignal): Promise<MediaItem | undefined> {
  ...
  public async downloadLine(
    ...
    if (response.response.status === 200) return response.data as FileStreamHttpResult;
  }
  ...
  public async transformItem(
    ...
    if (response.response.status === 202) return response.data as AcceptedTransformMediaItem;
  }
  ...
  private async *processStream<TValue, TValidator extends ZodType<TValue>>(
    ...
    const stream = response.body?.pipeThrough(new TextDecoderStream('UTF-8', { fatal: false, ignoreBOM: true }), {
      preventAbort: false,
    }) as Stream | undefined;
    if (stream !== undefined) {
      if (isAsyncIterable(stream)) {
        return yield* this.iterateStream<TValue, TValidator>(stream, responseValidator, signal);
      }

      return yield* this.whileStreamHasValues<TValue, TValidator>(stream, responseValidator, signal);
    }
  }
  ...
  private async *whileStreamHasValues<TValue, TValidator extends ZodType<TValue>>(
    stream: ReadableStream<string>,
    validator: TValidator,
    signal?: AbortSignal,
  ): AsyncGenerator<TValue> {
  ...
  private async *processChunk<TValue, TValidator extends ZodType<TValue>>(
    ...
    const lines = chunk
      .trim()
      .split('\n')
      .filter((line) => line.trim().length > 0 && line.startsWith('{') && line.endsWith('}'));
