...
public static class MediaDownloadEndpoints
{
    ...
    public static void MapDownloadEndpoints(this RouteGroupBuilder app)
    {
        ...
        group
            .MapGet("item/{mediaId:guid:required}/{width:int:required}/{height:int:required}", DownloadItem)
    ...
    )
    {
        ...
        var response = await service.DownloadMedia(mediaId, width, height, blur, format, cancellationToken);
