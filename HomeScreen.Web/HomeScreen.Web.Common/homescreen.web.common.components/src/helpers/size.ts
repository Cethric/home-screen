...
export const useImageSize: (args: { image: Image; border?: number; maxSize?: number }) => {
  ...
  const computedRatio = computed(() => {
    const base = toValue(width) > toValue(height) ? toValue(height) - border : toValue(width) - border;
