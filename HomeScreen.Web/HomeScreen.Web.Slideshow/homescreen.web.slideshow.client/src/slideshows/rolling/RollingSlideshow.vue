...
import { type Direction, Directions, type Image, type WeatherForecast } from '@homescreen/web-common-components';
import { v4 as uuid } from 'uuid';
import { computed } from 'vue';
import { DateTimeWeatherComboAsync } from '@/components/DateTimeWeatherComboAsync';
import { DateTimeWeatherComboKinds, RollingDirections } from '@/components/properties';
...
const actualDirection = computed(() =>
  props.direction === Directions.random ? choice([Directions.vertical, Directions.horizontal]) : props.direction,
);
...
const images = computed(() => (hasImages.value ? Object.values(props.images) : []));

const imageGroups = computed(() =>
  hasImages.value
    ? Array.from({ length: props.count }).map((_, idx) => ({
        images: images.value.slice(idx * size.value, idx * size.value + size.value),
        id: uuid(),
        direction: idx % 2 === 0 ? RollingDirections.backward : RollingDirections.forward,
      }))
    : [],
);
</script>
