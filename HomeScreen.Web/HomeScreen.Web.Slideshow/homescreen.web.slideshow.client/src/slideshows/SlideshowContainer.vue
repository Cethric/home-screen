...
const { execute, isReady } = useAsyncState(
  async (signal?: AbortSignal) => {
    ...
    let loaded = 0;
    let media: AsyncGenerator<MediaItem | undefined> = (async function* () {})();
    try {
    ...
    for await (const item of media) {
      ...
      if (item && item.id !== undefined) {
        const duplicateIndex = images.value.findIndex((img: Image) => img.id === item.id);
        if (duplicateIndex >= 0) {
        ...
        await nextTick(() => {
          images.value.push(transformMediaItemToImage(item as Required<MediaItem>));
          ++loaded;
          progress.value = loaded / currentTotal.value;
        });
      }
