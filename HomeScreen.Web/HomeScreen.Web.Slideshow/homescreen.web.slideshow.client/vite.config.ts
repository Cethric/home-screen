...
const makeServerConfig = (): UserConfig['server'] => {
  ...
  let target = env.ASPNETCORE_HTTPS_PORT ? `https://localhost:${env.ASPNETCORE_HTTPS_PORT}` : 'https://localhost:7163';
  ...
  if (env.ConnectionStrings__OtelCollector) {
    const connection = parseConnectionString(env.ConnectionStrings__OtelCollector);
...
export default defineConfig(({ command }) => {
  return {
    plugins: [vue(), vueDevTools({ launchEditor: 'webstorm' }), tailwindcss()],
    build: {
      rollupOptions: {
        output: {
          manualChunks: {
